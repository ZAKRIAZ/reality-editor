...
class RealityEngine {
  ...
  frameFromHands(hands, dt) {
    ...
    if (!open.length && this._frame) return;   // fist/pinch mid-lock: hold the framed pose
    const use = open.length ? open : hands;
    const A = Math.max(0.2, this._m.dw / Math.max(1, this._m.dh));
    const k = 1 - Math.pow(0.000001, dt);
    if (use.length >= 2 && use[0].tips && use[1].tips && this.shapeMode !== 'circle') {
      ...
      const target = [aTop[0], bTop[0], bTop[1], aTop[1]];   // TL, TR, BR, BL
      if (!this._frame || !this._frame.quad) this._frame = { quad: true, corners: target.map(p => ({ x: p.x, y: p.y })) };
      const f = this._frame;
      for (let n = 0; n < 4; n++) {
    ...
    // one hand ellipse — or two-hand circle when shapeMode is 'circle'
